...
- **Rotating authentication** — server-side PKCE login stores one site-scoped, non-autoloaded `{ email, uuid, access_token, refresh_token }` credential tuple. Every refreshable protected API call can refresh a rejected access token and retry once. One DB-backed auth lock serializes each complete remote Login and Refresh flow plus Logout; Logout makes one short best-effort server-session revocation and always continues with local cleanup. If an attempted refresh fails for any reason, credentials still belonging to that server-issued Login-session UUID are removed and Login is required again.
...
├── uninstall.php              Cleanup on plugin deletion: best-effort revokes the remote WordPress client, disables the auth epoch, purges credentials under the lifecycle lock, and never directly removes a foreign lock
...
└── includes/
    ...
    ├── client/
    │   ├── lihi-client-interface.php       Auth contract for login/register/authorization-code exchange/refresh/one-shot logout plus protected profile, domain-option, work-group, passthrough, and short-URL endpoints; refreshable protected methods accept an access fallback and retry themselves at most once
    │   ├── lihi-client.php                 HTTP client; Login and Register send the trusted WordPress hostname, Login adds credentials + PKCE challenge, Register maps the stable country-unavailable reason, auth success requires HTTP 2xx plus result:true, token exchange receives the server UUID/access/refresh tuple, refresh rotates it, Logout sends one bearer-authenticated request without refresh, and other protected calls—including nullable work-group switches—fall back only after a decoded JSON HTTP 401
...
4. Every refreshable protected client method receives the current access token and a fallback callback. On a valid JSON HTTP 401 response only, the fallback acquires that same site-wide auth lease, fresh-reads the tuple, and either reuses a concurrent request's newer access token or retains the lease while exchanging the stored identifier + refresh token, rechecking the activation generation and renewing after the remote response, validating it, and atomically replacing the bundle. Concurrent Refresh waiters and Logout allow up to 18 seconds for that 15-second HTTP leg plus persistence. The original protected endpoint is retried exactly once. Malformed or non-JSON 401 responses fail closed without refresh. Logout instead holds the lease, makes one bearer-authenticated `/auth/logout` attempt with a five-second timeout and no refresh, ignores any remote/read failure, and still deletes the local tuple before releasing.
...
10. Activation, deactivation, and uninstall use a complete lifecycle transition. It removes the epoch before waiting, acquires the shared lock within 22 seconds, and repeats disable under the lock. Uninstall then fresh-reads the final stored access token under that same lease and makes the one-shot five-second remote Logout attempt immediately before credential purge; every callback/read/remote error is ignored and purge still runs. Activation instead purges and installs a fresh epoch, while deactivation confirms authentication remains disabled. This ordering prevents remote revocation from racing an in-flight Login or Refresh and keeps the 27-second wait/HTTP upper bound below the common 30-second PHP execution limit. Login and Refresh recheck the fence after a remote response and therefore cannot renew or begin another HTTP leg after lifecycle cleanup starts. Self-loading callbacks do not depend on admin bootstrap. If the transition fails, best-effort disable/delete attempts are isolated so hook execution continues; an absent epoch keeps any undeleted secret unusable, and a foreign lock remains untouched. This development branch reads only the current bundle shape.
